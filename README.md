...
_An introduction to operational semantics: values vs. expressions, basic error handling,_
...
gives inherent lexical scoping.
...
**The judgment** &rarr; `eval(e, env) = v` &rarr; _"expression `e`, under environment `env`,_
_produces value `v`."_
...
└── mapexercise/   Map practice for the environment
```
...
- **Use watch mode.** Inside the shell, `~problem6/test` re-runs that problem's
...
- **`???` keeps the build alive.** An _unimplemented_ case (with `???`) still compiles, so
  ...
  build. If `problem6/test` suddenly won't start, check the compile error; it's
  often in a different package.
- **One subproject per problem = isolation.** `problem6.Const` and `problem7.Const` are
  different projects, so each problem redefines `Expr` / `Value` / `eval` freely. Target one
  with `testOnly problem6.*`.
- **Run a single test case** by name with ScalaTest's `-z` filter:
```bash
  sbt "problem6/testOnly *"
```
...
- **Error Handling**: Type mismatches produce `ErrorValue`, they propagate up
