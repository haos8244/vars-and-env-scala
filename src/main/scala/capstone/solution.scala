...
/* =============================================================================
 * CAPSTONE: Basic Lettuce Interp (Const, Plus, Gt, Ident, Let, IfThenElse, And)
 * =============================================================================
 *
 * NOTE:
 *   Implement `eval(env)` for the small Lettuce expression language. This
 *   pulls together everything: values vs. expressions, error
 *   handling, the environment, variables (Ident), bindings (Let), conditionals,
 *   and short-circuiting And.
 *
 * PERF: THE GRAMMAR (abstract syntax)
 *
 ...
 *     Otherwise bind id to that value in a NEW environment
 *     and evaluate e2 under the new environment. Return e2's value
 *
 *     WARNING: the original env must not be mutated
 *
 *   IfThenElse(c, t, f):
 *     Evaluate c.
 ...
 *   And(e1, e2):
 *     Evaluate e1.
 *       BoolValue(false) -> return BoolValue(false), not evaluating e2 (short-circuit)
